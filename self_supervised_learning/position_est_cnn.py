...
model_state_dict = torch.load('results/unlabeled/cnn/best_self_supervised_model_cnn.pth')
...
# Output shape of the encoder from your SelfSupervisedModel
encoder_output_shape = (64, 14, 231)

# Define input_size to match the shape of the encoder's output
input_size = encoder_output_shape[0] * encoder_output_shape[1] * encoder_output_shape[2]
feature_size = encoder_output_shape[0]  # Number of channels (64 in this case)

# Update the combined model with the modified PositionEstimationModel
combined_model = CombinedPositionEstimationModel(pretrained_encoder, input_size=input_size).to(device)
...
for epoch in range(num_epochs):
    ...
    for data, labels in train_dataloader:
        ...
        predictions = combined_model(data)
...
if best_model_weights is not None:
    torch.save(best_model_weights, 'results/unlabeled/cnn/best_combined_position_estimation_modelCNN.pth')
...
plt.legend()
plt.grid(True)
plt.savefig('loss_curvesCNN.png')
plt.show()
...
model_weights_path = "results/unlabeled/cnn/best_combined_position_estimation_modelCNN.pth"
