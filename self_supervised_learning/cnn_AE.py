...
for epoch in range(num_epochs):
    ...
    for data, snr_data in train_dataloader:
        optimizer.zero_grad()
        # Add a channel dimension (batch size, channels, height, width)
    ...
    with torch.no_grad():
        for val_data, val_snr_data in val_dataloader:
            val_inputs = val_data.unsqueeze(1).to(device)
...
torch.save(best_model_state_dict, 'results/unlabeled/cnn/best_self_supervised_model_cnn.pth')
...
plt.legend()
plt.grid(True)
plt.savefig('results/unlabeled/cnn/loss_curvesAEunlabelled_cnn.png')
plt.show()
